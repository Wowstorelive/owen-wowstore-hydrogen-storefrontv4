...
function home({homeData}: {homeData?: any}): SeoConfig {
  return {
    title: `${homeData?.seo?.title ?? 'Home'}`,
    titleTemplate: `%s | ${config.storeName}`,
    description: `${
      homeData?.seo?.description ??
      'Owen theme - The best of Shopify Hydrogen templates'
    }`,
    robots: {
      noIndex: false,
      noFollow: false,
